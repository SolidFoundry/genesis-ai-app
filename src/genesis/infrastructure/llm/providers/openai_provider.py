"""
Genesis AI 应用 OpenAI 提供商
============================

本模块实现 OpenAI API 的 LLM 提供商，遵循 LLMProviderInterface 接口。
支持聊天补全、文本生成、模型查询等功能。
"""
...
class OpenAIProvider(LLMProviderInterface):
    ...
    def _get_api_key(self) -> str:
        ...
        if not api_key:
            raise LLMProviderAuthenticationError("OpenAI API密钥未配置，请设置 OPENAI_API_KEY 环境变量")
        return api_key
        ...
        **kwargs
        ...
        try:
            ...
            if response.status_code == 401:
                raise LLMProviderAuthenticationError("OpenAI API认证失败")
            elif response.status_code == 429:
                raise LLMProviderRateLimitError("OpenAI API请求频率限制")
            elif response.status_code >= 500:
                raise LLMProviderServiceUnavailableError("OpenAI服务不可用")
            elif response.status_code != 200:
                raise LLMProviderException(f"OpenAI API请求失败: {response.status_code}")
            ...
            raise LLMProviderServiceUnavailableError("OpenAI API请求超时")
        except httpx.NetworkError as e:
            raise LLMProviderServiceUnavailableError(f"OpenAI API网络错误: {e}")
        except Exception as e:
            if isinstance(e, LLMProviderException):
                raise
            raise LLMProviderException(f"OpenAI API请求异常: {e}")
    ...
    async def generate_text(
        ...
        **kwargs
        ...
        if not choices:
            raise LLMProviderException("OpenAI API返回空响应")
